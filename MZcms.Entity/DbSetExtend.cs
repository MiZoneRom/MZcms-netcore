...
namespace MZcms.Entity
{
    public static class DbSetExtend
    {
        ...
        /// <returns></returns>
        public static IQueryable<TEntity> FindBy<TEntity, TKey>(this DbSet<TEntity> dbSet, Expression<Func<TEntity, bool>> where, Expression<Func<TEntity, TKey>> order, int pageNumber, int pageSize, out int total) where TEntity : BaseModel
        {
            total = dbSet.Count(where);
            return dbSet.Where(where).OrderBy(order).Skip((pageNumber - 1) * pageSize).Take(pageSize);
        }
        ...
        /// <returns></returns>
        public static IQueryable<TEntity> GetPage<TEntity>(this IQueryable<TEntity> entities, Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>> orderBy,
            out int total, int pageNumber = 1, int pageSize = 20)
        {
            //if (orderBy == null)
            //{
        ...
        //        entities.Add(dbSet.FirstOrDefault(p => p.Id == id));
